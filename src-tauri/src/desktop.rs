...
const OPEN_FILE_DEDUP_WINDOW: Duration = Duration::from_secs(3);

/// Stores external file-open requests until the frontend is ready to consume
/// them. macOS can deliver the same Finder "Open With" action through more
/// than one native event, so requests are deduplicated before import.
static OPEN_FILE_STATE: Mutex<OpenFileState> = Mutex::new(OpenFileState::new());

/// macOS can deliver `RunEvent::Opened` before Tauri emits `Ready` and before
/// `setup_desktop` has installed state/plugins. Keep early file events queued
/// until it is safe to touch windows or emit frontend events.
...
impl OpenFileState {
    ...
    fn enqueue(&mut self, file_path: String, now: Instant) -> OpenFileRequestStatus {
        self.prune_recent(now);
        let key = open_file_key(&file_path);
        if self.pending_keys.iter().any(|item| item == &key) {
            return OpenFileRequestStatus::AlreadyPending;
        }
        if self.recent.iter().any(|(item, _)| item == &key) {
            return OpenFileRequestStatus::RecentDuplicate;
        }

        self.pending.push(file_path);
        self.pending_keys.push(key.clone());
        self.recent.push((key, now));
        OpenFileRequestStatus::Queued
    }
    ...
    fn prune_recent(&mut self, now: Instant) {
        self.recent.retain(|(_, seen_at)| {
            now.saturating_duration_since(*seen_at) <= OPEN_FILE_DEDUP_WINDOW
        });
    }
}
...
#[derive(Debug)]
struct AppQuitPrepareSession {
    request_id: String,
    reports: BTreeMap<String, AppQuitPrepareWindowStatus>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
enum AppQuitPrepareWindowStatus {
    Pending,
    Ready,
    Failed(String),
}
...
struct RuntimeState {
    ...
    app_quit_prepare: Mutex<Option<AppQuitPrepareSession>>,
    #[cfg(desktop)]
...
impl RuntimeState {
    ...
    fn begin_app_quit_prepare<I>(&self, labels: I) -> String
    where
        I: IntoIterator<Item = String>,
    {
        ...
        let reports = labels
            .into_iter()
            .map(|label| (label, AppQuitPrepareWindowStatus::Pending))
            .collect();

        if let Ok(mut guard) = self.app_quit_prepare.lock() {
            *guard = Some(AppQuitPrepareSession {
    ...
    fn report_app_quit_prepare(
        &self,
        request_id: &str,
        window_label: &str,
        status: AppQuitPrepareWindowStatus,
    ) -> AppQuitPrepareState {
        let Ok(mut guard) = self.app_quit_prepare.lock() else {
            return AppQuitPrepareState::Unknown;
        };
        ...
        if let Some(report) = session.reports.get_mut(window_label) {
            *report = status;
        }
        app_quit_prepare_state(session)
    }

    fn poll_app_quit_prepare(&self, request_id: &str) -> AppQuitPrepareState {
        let Ok(guard) = self.app_quit_prepare.lock() else {
            return AppQuitPrepareState::Unknown;
        };
    ...
    fn clear_app_quit_prepare(&self, request_id: &str) {
        let Ok(mut guard) = self.app_quit_prepare.lock() else {
            return;
        };
        if guard
...
fn app_quit_prepare_state(session: &AppQuitPrepareSession) -> AppQuitPrepareState {
    if session
        .reports
        .values()
        .any(|status| matches!(status, AppQuitPrepareWindowStatus::Failed(_)))
    {
        AppQuitPrepareState::Failed
    } else if session
        .reports
        .values()
        .all(|status| matches!(status, AppQuitPrepareWindowStatus::Ready))
    {
        AppQuitPrepareState::Ready
    } else {
        AppQuitPrepareState::Pending
    }
}
...
pub fn extract_file_arg(args: &[String]) -> Option<String> {
    args.iter()
        .find(|arg| is_supported_open_file_path(arg))
        .cloned()
}

pub fn extract_file_url(url: &Url) -> Option<String> {
    if url.scheme() != "file" {
        return None;
    }
    let path = url.to_file_path().ok()?;
    let path = path.to_string_lossy().to_string();
    is_supported_open_file_path(&path).then_some(path)
}

pub fn handle_open_file_request(app: &AppHandle, file_path: String) {
    let status = enqueue_open_file_request(file_path);
    if !desktop_ready() {
        return;
    }

    if status.is_some_and(should_notify_open_file_request) {
        let _ = app.emit("open-external-file", ());
    }
    let _ = show_main_window(app);
}

fn enqueue_open_file_request(file_path: String) -> Option<OpenFileRequestStatus> {
    OPEN_FILE_STATE
        .lock()
        .ok()
        .map(|mut state| state.enqueue(file_path, Instant::now()))
}

fn should_notify_open_file_request(status: OpenFileRequestStatus) -> bool {
    matches!(
        status,
        OpenFileRequestStatus::Queued | OpenFileRequestStatus::AlreadyPending
    )
}

fn has_pending_open_file_requests() -> bool {
    OPEN_FILE_STATE
        .lock()
        .map(|state| state.has_pending())
        .unwrap_or(false)
}

fn desktop_ready() -> bool {
    DESKTOP_READY.load(Ordering::SeqCst)
}

fn notify_pending_open_file_requests(app: &AppHandle) {
    if !desktop_ready() || !has_pending_open_file_requests() {
        return;
    }
...
fn is_supported_open_file_path(path: &str) -> bool {
...
fn open_file_key(file_path: &str) -> String {
...
/// Takes queued external file-open requests, consuming them so subsequent calls
/// return only new requests. Called by the frontend after initialization and
/// when it receives an `open-external-file` notification.
pub fn take_startup_files() -> Vec<String> {
    OPEN_FILE_STATE
        .lock()
...
pub fn setup_desktop(app: &mut App) -> Result<(), Box<dyn Error>> {
    ...
    if let Some(file_path) = extract_file_arg(&args) {
        let _ = enqueue_open_file_request(file_path);
    }

    if !args.iter().any(|a| a == "--silent") || has_pending_open_file_requests() {
        if let Err(error) = show_main_window(app.handle()) {
            eprintln!("failed to show main window on startup: {error}");
        }
    }

    notify_pending_open_file_requests(app.handle());
...
pub fn handle_window_event(window: &Window, event: &WindowEvent) {
    ...
    match main_window_close_action(app_is_exiting(window.app_handle()), close_to_tray_enabled()) {
        ...
        MainWindowCloseAction::RequestFrontendSave => {
            api.prevent_close();
...
fn main_window_close_action(app_is_exiting: bool, _close_to_tray: bool) -> MainWindowCloseAction {
    if app_is_exiting {
        MainWindowCloseAction::AllowClose
    } else {
        MainWindowCloseAction::RequestFrontendSave
...
pub fn request_main_window_close(app: &AppHandle) -> Result<(), AppError> {
...
pub fn request_app_quit(app: &AppHandle) -> Result<(), AppError> {
    let windows = app.webview_windows();
    let labels = windows
        .iter()
        .filter_map(|(label, window)| {
            if label.as_str() == MAIN_WINDOW_LABEL
                || (label.starts_with("notepad-") && window.is_visible().unwrap_or(false))
            {
                Some(label.clone())
            } else {
                None
    ...
    if labels.is_empty() {
        return request_app_exit(app);
    }

    let Some(state) = app.try_state::<RuntimeState>() else {
        return request_app_exit(app);
    };
    let request_id = state.begin_app_quit_prepare(labels.clone());
    let payload = AppQuitRequestPayload {
        request_id: request_id.clone(),
    };

    for label in labels {
        if let Some(window) = windows.get(&label) {
            if let Err(error) = window.emit(APP_QUIT_REQUESTED_EVENT, &payload) {
                let status =
                    AppQuitPrepareWindowStatus::Failed(format!("无法通知窗口保存：{error}"));
                let _ = state.report_app_quit_prepare(&request_id, &label, status);
            }
        }
    }

    if state.poll_app_quit_prepare(&request_id) == AppQuitPrepareState::Ready {
        state.clear_app_quit_prepare(&request_id);
        return request_app_exit(app);
    }
...
pub fn report_app_quit_preparation(
    app: &AppHandle,
    request_id: &str,
    window_label: &str,
    status: &str,
    ...
    let status = match status {
        "ready" => AppQuitPrepareWindowStatus::Ready,
        "failed" => AppQuitPrepareWindowStatus::Failed(message.unwrap_or_default()),
        _ => {
            return Err(AppError {
                code: "invalidQuitPrepareStatus".into(),
                message: "invalid app quit prepare status".into(),
                details: Default::default(),
            });
        }
    };

    match state.report_app_quit_prepare(request_id, window_label, status) {
        AppQuitPrepareState::Ready => {
            state.clear_app_quit_prepare(request_id);
            request_app_exit(app)?;
        }
        ...
        AppQuitPrepareState::Pending | AppQuitPrepareState::Unknown => {}
    }

    Ok(())
}

pub fn request_app_exit(app: &AppHandle) -> Result<(), AppError> {
...
mod tests {
    ...
    #[test]
    fn extracts_supported_file_args() {
        assert_eq!(
            extract_file_arg(&["mininote".into(), "/tmp/note.md".into()]),
            Some("/tmp/note.md".into())
        );
        assert_eq!(
            extract_file_arg(&["mininote".into(), "/tmp/readme.pdf".into()]),
            None
        );
    }

    #[test]
    fn extracts_supported_file_urls() {
        let markdown_url = Url::parse("file:///tmp/Meeting%20Notes.md").expect("valid file url");
        assert_eq!(
            extract_file_url(&markdown_url),
            Some("/tmp/Meeting Notes.md".into())
        );

        let web_url = Url::parse("https://example.com/note.md").expect("valid web url");
        assert_eq!(extract_file_url(&web_url), None);

        let unsupported_url = Url::parse("file:///tmp/readme.pdf").expect("valid file url");
        assert_eq!(extract_file_url(&unsupported_url), None);
    }

    #[test]
    fn queues_external_open_requests_once_for_repeated_native_events() {
        let mut state = OpenFileState::new();
        let now = Instant::now();

        assert_eq!(
            state.enqueue("/tmp/note.md".into(), now),
            OpenFileRequestStatus::Queued
        );
        for _ in 0..8 {
            assert_eq!(
                state.enqueue("/tmp/note.md".into(), now),
                OpenFileRequestStatus::AlreadyPending
            );
        }

        assert_eq!(state.drain(), vec!["/tmp/note.md"]);
        assert!(state.drain().is_empty());
    }

    #[test]
    fn keeps_recent_external_open_requests_deduped_after_drain() {
        let mut state = OpenFileState::new();
        let now = Instant::now();
        ...
        assert_eq!(state.drain(), vec!["/tmp/note.md"]);
        assert_eq!(
            state.enqueue("/tmp/note.md".into(), now + Duration::from_secs(1)),
            OpenFileRequestStatus::RecentDuplicate
        );
        assert_eq!(
            state.enqueue("/tmp/note.md".into(), now + Duration::from_secs(4)),
            OpenFileRequestStatus::Queued
        );
    }

    #[test]
    fn queues_distinct_external_open_requests_together() {
        let mut state = OpenFileState::new();
        let now = Instant::now();

        assert_eq!(
            state.enqueue("/tmp/first.md".into(), now),
            OpenFileRequestStatus::Queued
        );
        assert_eq!(
            state.enqueue("/tmp/second.txt".into(), now),
            OpenFileRequestStatus::Queued
        );

        assert_eq!(state.drain(), vec!["/tmp/first.md", "/tmp/second.txt"]);
    }
    ...
    #[test]
    fn asks_frontend_to_save_when_main_window_closes_without_close_to_tray() {
        assert_eq!(
    ...
    #[test]
    fn asks_frontend_to_save_when_main_window_closes_to_tray() {
        assert_eq!(
    ...
    #[test]
    fn allows_main_window_close_after_app_exit_is_approved() {
        assert_eq!(
    ...
    #[test]
    fn app_quit_prepare_waits_for_all_editable_windows() {
        let state = RuntimeState::default();
        let request_id = state.begin_app_quit_prepare(vec!["main".into(), "notepad-1".into()]);

        assert_eq!(
            state.poll_app_quit_prepare(&request_id),
            AppQuitPrepareState::Pending
        );
        assert_eq!(
            state.report_app_quit_prepare(&request_id, "main", AppQuitPrepareWindowStatus::Ready),
            AppQuitPrepareState::Pending
        );
        assert_eq!(
            state.report_app_quit_prepare(
                &request_id,
                "notepad-1",
                AppQuitPrepareWindowStatus::Ready
            ),
            AppQuitPrepareState::Ready
        );
    }
    ...
    #[test]
    fn app_quit_prepare_stops_on_failed_window() {
        let state = RuntimeState::default();
        let request_id = state.begin_app_quit_prepare(vec!["main".into(), "notepad-1".into()]);
        ...
        assert_eq!(
            state.report_app_quit_prepare(
                &request_id,
                "notepad-1",
                AppQuitPrepareWindowStatus::Failed("save failed".into())
            ),
            AppQuitPrepareState::Failed
        );
    }
