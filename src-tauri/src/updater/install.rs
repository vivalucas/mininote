...
const UPDATE_HELPER_PATH_ENV: &str = "MININOTE_UPDATE_HELPER_PATH";
const UPDATE_HELPER_MODE_ENV: &str = "MININOTE_UPDATE_HELPER_MODE";
...
impl UpdateInstallService<ProcessInstallExecutor> {
    pub(crate) fn from_env() -> Self {
        Self {
            helper_source_override: env_text(UPDATE_HELPER_PATH_ENV).map(PathBuf::from),
...
fn helper_mode_from_env() -> helper::UpdateHelperMode {
    env_text(UPDATE_HELPER_MODE_ENV)
        .as_deref()
...
fn stage_windows_helper_copy(paths: &UpdatePaths, source_path: &Path) -> Result<PathBuf, AppError> {
    ...
    let source_ext = source_path
        ...
        .and_then(|ext| ext.to_str())
        .unwrap_or("exe");
    let staged_exe = staged_dir.join(format!("mininote-update-helper.{source_ext}"));
...
mod tests {
    ...
    fn write_bundle_with_helper(bundle: &Path) {
        let app_binary = bundle.join("Contents").join("MacOS").join("mininote");
    ...
    fn launches_real_install_helper_after_staging_copy() {
        ...
        let bundle = paths.root_dir().join("MiniNote.app");
    ...
    fn resolves_macos_helper_and_target_from_fallback_search_dir() {
        ...
        let actual_bundle = fallback_dir.join("MiniNote.app");
        write_bundle_with_helper(&actual_bundle);

        let stale_bundle = paths.root_dir().join("gone").join("MiniNote.app");
    ...
    fn stages_windows_helper_with_adjacent_dll_dependencies() {
        ...
        let source = source_dir.join("mininote.exe");
        ...
        assert_eq!(
            ...
            Some("mininote-update-helper.exe")
        );
