...
#[cfg(target_os = "windows")]
pub const HELPER_BINARY_NAME: &str = "mininote-update-helper.exe";
#[cfg(not(target_os = "windows"))]
pub const HELPER_BINARY_NAME: &str = "mininote-update-helper";
...
const UPDATE_STAGE_PREFIX: &str = ".mininote-update-stage-";
#[cfg(target_os = "macos")]
const MOUNT_STAGE_PREFIX: &str = ".mininote-mounted-dmg-";
...
fn windows_rollback_storage_parent(
    ...
    Ok(std::env::temp_dir().join("mininote-update-rollback"))
}
...
fn unique_temp_path(parent: &Path, stem: &str, extension: Option<&str>) -> PathBuf {
    let unique = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    let mut file_name = format!(".mininote-{stem}-{unique}");
...
fn write_log_header(
    ...
    write_log_line(file, "mininote update helper")?;
...
mod tests {
    ...
    fn parses_strict_arguments() {
        let args: Vec<OsString> = vec![
            ...
            OsString::from("42"),
            OsString::from("--target-path"),
            OsString::from("/Applications/MiniNote.app"),
            OsString::from("--log-path"),
    ...
    fn expected_process_name_uses_macos_bundle_executable() {
        ...
        let bundle = root.join("MiniNote.app");
        let macos_dir = bundle.join("Contents").join("MacOS");
        fs::create_dir_all(&macos_dir).expect("create macos dir");
        fs::write(macos_dir.join("mininote"), b"binary").expect("write binary");

        assert_eq!(expected_process_name(&bundle).as_deref(), Some("mininote"));
        assert_eq!(
            expected_process_name(&root.join("mininote")).as_deref(),
            Some("mininote")
        );
    }
    ...
    fn cleanup_applied_update_retains_macos_rollback_stage() {
        ...
        let stage_root = root.join(".mininote-update-stage-test");
    ...
    fn rollback_windows_update_restores_original_executable() {
        ...
        let target_path = install_dir.join("mininote.exe");
        ...
        fs::write(backup_dir.join("mininote.exe"), b"old version").expect("write backup");
    ...
    fn windows_rollback_storage_parent_moves_out_of_install_dir() {
        ...
        let mut command = helper_command(&root);
        command.install_kind = InstallKind::WindowsNsis;
        command.target_path = install_dir.join("mininote.exe");
        command.ready_path = staging_dir.join("helper.ready");
        ...
        assert!(
            backup_parent.ends_with("mininote-update-rollback"),
...
HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Uninstall\MiniNote
    DisplayIcon    REG_SZ    C:\Program Files\MiniNote\mininote.exe,0
    InstallLocation    REG_SZ    C:\Program Files\MiniNote
"#;

        let key = parse_windows_install_registry_key_from_search_output(
            output,
            "mininote.exe",
            Some(Path::new(r"C:\Program Files\MiniNote\mininote.exe")),
        )
        .expect("resolve matching uninstall key");

        assert_eq!(
            key,
            r"HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Uninstall\MiniNote"
        );
    }
...
HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Uninstall\MiniNote
    DisplayIcon    REG_SZ    C:\Program Files\MiniNote\mininote.exe,0
    InstallLocation    REG_SZ    C:\Program Files\MiniNote
    QuietUninstallString    REG_SZ    "C:\Program Files\MiniNote\uninstall.exe" /S
    UninstallString    REG_SZ    "C:\Program Files\MiniNote\uninstall.exe"
"#;
        ...
        assert_eq!(
            ...
            Some(normalize_windows_path(
                r"C:\Program Files\MiniNote\mininote.exe"
            ))
        );
        assert_eq!(
            record.install_dir,
            Some(PathBuf::from(r"C:\Program Files\MiniNote"))
        );
        assert_eq!(
            record.quiet_uninstall_command.as_deref(),
            Some(r#""C:\Program Files\MiniNote\uninstall.exe" /S"#)
        );
        assert_eq!(
            record.uninstall_command.as_deref(),
            Some(r#""C:\Program Files\MiniNote\uninstall.exe""#)
        );
    }
    ...
    fn builds_silent_windows_uninstall_command_from_registry_record() {
        let nsis_record = WindowsRegistryInstallRecord {
            key_path: "HKLM\\Software\\MiniNote".into(),
            launch_target: Some(PathBuf::from(r"C:\Program Files\MiniNote\mininote.exe")),
            install_dir: Some(PathBuf::from(r"C:\Program Files\MiniNote")),
            quiet_uninstall_command: None,
            uninstall_command: Some(r#""C:\Program Files\MiniNote\uninstall.exe""#.into()),
        };
        let msi_record = WindowsRegistryInstallRecord {
            key_path: "HKLM\\Software\\MiniNoteMsi".into(),
            launch_target: Some(PathBuf::from(r"C:\Program Files\MiniNote\mininote.exe")),
            install_dir: Some(PathBuf::from(r"C:\Program Files\MiniNote")),
            quiet_uninstall_command: None,
    ...
    fn mounted_dmg_selection_prefers_root_app_bundle() {
        ...
        let root_bundle = mounted_root.join("MiniNote.app");
        let stale_bundle = fake_applications
            .join(".mininote-update-stage-123")
            .join("MiniNote.app");
        ...
        let selected =
            select_app_bundle_from_mounted_dmg(&mounted_root, Some("MiniNote.app"), &mut log)
    ...
    fn bundle_search_skips_symlink_dirs_and_update_stage_dirs() {
        ...
        let valid_bundle = extracted_root.join("Nested").join("MiniNote.app");
        let stage_bundle = extracted_root
            .join(".mininote-update-stage-123")
            .join("MiniNote.app");
        let symlink_target = root.join("linked");
        let symlink_bundle = symlink_target.join("MiniNote.app");
