...
mod tests {
    ...
    fn detects_windows_portable_installation() {
        let install_kind = detect_install_kind(
            Os::Windows,
            Some(Path::new(r"D:\Apps\MiniNote\mininote.exe")),
        );
    ...
    fn detects_macos_app_bundle() {
        let bundle = find_macos_app_bundle(Path::new(
            "/Applications/MiniNote.app/Contents/MacOS/mininote",
        ));

        assert_eq!(bundle, Some(PathBuf::from("/Applications/MiniNote.app")));
        assert_eq!(
            detect_install_kind(
                Os::Macos,
                Some(Path::new(
    ...
    fn treats_unbundled_macos_binary_as_unknown_install_kind() {
        let install_kind =
            detect_install_kind(Os::Macos, Some(Path::new("/Users/test/dev/mininote")));
