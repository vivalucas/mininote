...
import type { Note, NoteMetadata, SaveNoteRequest, SyncSourceRequest } from "./types";
...
export function syncNoteSourceFile(id: string, request: SyncSourceRequest): Promise<NoteMetadata> {
  return invoke("notes_sync_source_file", { id, request });
}

export function reloadNoteSourceFile(id: string): Promise<Note> {
  return invoke("notes_reload_source_file", { id });
}
...
function getLocalizedAppErrorMessage(
  ...
  switch (appError.code) {
    ...
    case "sourceFileConflict":
      return translate("main.sourceConflict.title", { defaultValue: "原文件已被修改" });
    case "sourceFileMissing":
      return translate("main.sourceSync.missing", {
        defaultValue: "原文件不存在或不可访问，当前内容已保存到 MiniNote",
      });
    case "fileTooLarge":
      return translate("errors.fileTooLarge", {
        maxMb: appError.details.maxMb ?? "10",
        defaultValue: "文件过大（上限 {{maxMb}} MB）",
      });
    case "imageTooLarge":
      return translate("errors.imageTooLarge", {
        maxMb: appError.details.maxMb ?? "30",
        defaultValue: "图片文件过大（上限 {{maxMb}} MB）",
      });
    case "unsupportedImageFormat":
      return translate("errors.unsupportedImageFormat", { defaultValue: "不支持的图片格式" });
