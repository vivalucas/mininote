...
export function SettingsPanel({
  ...
  const localeOptions = useMemo(
    (): Array<{ value: SupportedLocale; label: string }> =>
  ...
  const selectedLocale = normalizeLocale(config.locale) ?? DEFAULT_LOCALE;
    ...
    <aside className="w-[360px] h-full shrink-0 border-l border-paper-deep/30 bg-cloud/92 backdrop-blur-sm flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hidden px-4 py-4 space-y-5">
        ...
        <section className="space-y-2">
          ...
          />
          <ToggleRow
            label={t("settings.autoSave.externalFile", { defaultValue: "关联原文件自动同步" })}
          ...
          />
          <ToggleRow
            label={t("settings.renderHtmlMarkdown", { defaultValue: "安全渲染 HTML 标签" })}
...
function LocaleSelect({ options, value, onChange }: LocaleSelectProps) {
  return (
    <div className="relative">
      <select
        ...
        className="h-9 w-full appearance-none rounded-lg border border-paper-deep/45 bg-paper-warm/60 px-3 pr-9 text-[12px] font-body font-medium text-ink-soft transition-colors cursor-pointer hover:border-paper-deep/70 focus:border-bamboo/70 focus:bg-cloud/90"
      >
        {options.map((option) => (
