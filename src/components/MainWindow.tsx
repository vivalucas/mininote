...
import { DEFAULT_LOCALE, normalizeLocale } from "../locales/locale-whitelist";
...
const AboutPanel = lazy<ComponentType<AboutPanelProps>>(() =>
  import("./AboutPanel").then((module) => ({ default: module.AboutPanel })),
);
const SettingsPanel = lazy<ComponentType<SettingsPanelProps>>(() =>
...
function getDocumentFormat(filePath: string | null): string {
  ...
  if (extension === "mint") return "MiniNote";
...
export function pinTileButtonTitle(isPinned: boolean, translate: TFunction): string {
  return isPinned
    ? translate("main.editor.unpinTile", { defaultValue: "取消钉屏" })
    : translate("main.editor.pinToTile", { defaultValue: "钉到屏幕" });
}
...
export function MainWindow({
  ...
  const importLinkedFile = useCallback(
    async (filePath: string) => {
      setErrorMessage(null);
      try {
        const note = await importMarkdownPath(filePath, activeCategory);
        replaceNoteMetadata(note);
        applyNote(note);
      } catch (error) {
    ...
    [activeCategory, applyNote, replaceNoteMetadata],
  );

  const importQueuedExternalFiles = useCallback(async () => {
    const filePaths = await takeStartupFiles();
    for (const filePath of filePaths) {
      await importLinkedFile(filePath);
    }
  }, [importLinkedFile]);
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      try {
        ...
        if (!cancelled) {
          await importQueuedExternalFiles();
        }
      } catch (error) {
  ...
  const saveCurrentNote = useCallback(async () => {
    ...
    try {
      ...
      return shouldBlockLeaving ? null : note;
    } catch (error) {
      setSaveState("error");
      setErrorMessage(getErrorMessage(error));
      return null;
  ...
  useEffect(() => {
    const unlisten = listen("open-external-file", () => {
      void (async () => {
  ...
  }, [ensureCurrentNoteSaved, importQueuedExternalFiles]);
  ...
  }, [ensureCurrentNoteSaved, loadNote]);
  ...
  }, [applyNote, replaceNoteMetadata, sourceConflict]);
  ...
  useEffect(() => {
    const unlisten = listen("main-window-close-requested", () => {
      void closeMainWindowAfterSave().catch((error) => {
        setErrorMessage(getErrorMessage(error));
      });
    });
    return () => {
  ...
  useEffect(() => {
    const unlisten = listen<{ requestId: string }>("app-quit-requested", (event) => {
      const respond = async () => {
        const windowLabel = windowLabelRef.current || "main";
        ...
        const saved = await ensureCurrentNoteSaved();
        await reportAppQuitPreparation(
          event.payload.requestId,
          windowLabel,
          saved ? "ready" : "failed",
  ...
  const handleChooseNotesDir = async () => {
    ...
    try {
      ...
      if (!notesDir) return;
      if (notesDir !== settingsConfig.notesDir) {
        const confirmed = window.confirm(
          t("settings.notesDirSwitchConfirm", {
            defaultValue:
              "切换笔记目录只会切换本地笔记库，不会自动迁移当前目录里的笔记。确定继续吗？",
          }),
        );
        if (!confirmed) return;
      }
    ...
    (nextConfig: AppConfig) => {
      ...
      settingsSaveTimer.current = setTimeout(async () => {
        ...
        const normalizedConfig = {
          ...nextConfig,
          locale: normalizeLocale(nextConfig.locale) ?? DEFAULT_LOCALE,
  ...
  const handleExportNote = async (note: NoteMetadata, format: "markdown" | "mint") => {
    setErrorMessage(null);
    try {
  ...
  const handleNoteMenuAction = (action: NoteContextMenuAction) => {
    ...
    if (action === "exportMarkdown" || action === "exportMint") {
      setNoteMenuClosing(true);
      void handleExportNote(note, action === "exportMint" ? "mint" : "markdown");
      return;
    }
  ...
  const handlePinEntry = async () => {
    ...
    if (!isPinned && !(await ensureCurrentNoteSaved())) return;
      ...
      <div className="relative noise-bg bg-cloud overflow-hidden flex flex-col flex-1">
        ...
        {sourceConflict && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/18 backdrop-blur-[2px] px-4">
            <div className="w-full max-w-[420px] rounded-xl border border-paper-deep/50 bg-paper/95 shadow-[0_18px_60px_rgba(47,40,32,0.18)] px-5 py-4">
              <div className="flex items-start gap-3">
                <div className="mt-0.5 w-8 h-8 rounded-lg bg-amber-100/80 text-amber-700 flex items-center justify-center shrink-0">
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
            ...
            <div className="flex items-center justify-between px-4 h-10 border-b border-paper-deep/20 shrink-0 bg-paper/20">
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  disabled={!selectedId}
                  aria-label={pinTileButtonTitle(selectedTilePinned, t)}
                  ...
                  title={pinTileButtonTitle(selectedTilePinned, t)}
                >
                  <svg
                    width="13"
                    height="13"
            ...
            >
              ...
              <div className="flex items-center gap-3 mt-1.5">
                <span className="text-[10px] text-ink-ghost font-mono tabular-nums truncate max-w-[200px]">
                  {selectedSourcePath
                    ? t("main.sourceFile.label", {
                        path: selectedSourcePath,
                        defaultValue: "同步原文件 · {{path}}",
                      })
                    : selectedNote
